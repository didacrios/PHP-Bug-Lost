...
// for better initial memory value.
...
// Set to "all" (default) to include all types
...
// show or hide the shortcuts numbers on menu buttons
...
// Keep empty for no include log messages on admin mails
...
// check secret key
if (_bl_delete_vars == true and _bl_secrect_key == '_pbl_' and _bl_debug_on == true) {
...
function bl_link_file($file, $line) {
    ...
    if (_bl_file_browser != true) {
        ...
        if (!empty($file)) {
            if (PHP_OS == 'WINNT') {
                $href = str_replace(DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR, $file);
                $url  = str_replace(DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR, _bl_path);
            }else {
                $href = $file;
                $url  = _bl_path;
            }

            $file_name = '<a href="javascript:void(0);" onclick="bl_load_file(\''.$href.'\', \''.$line.'\', \''.$url.'\', \''._bl_secrect_key.'\')">'.$file.'</a>';
        }else {
...
class PDO extends PDO {
  ...
  /**
   * PDO::query()
   * send PDO::query() with debug
   *
   * @param mixed $params same parameters of query de PDO
   *
   * @return mixed
   */
    	  ...
    	  $insert_id = parent::lastInsertId();
    		...
    		if (substr($q, 0, 6) == 'select') { // if is select get num rows
    			...
    			if (_bl_explain_sql && $error == null) {
    				...
    				$explain = $sql_explain->fetch(PDO::FETCH_ASSOC);
 ...
 */
function bl_debug($return = false) {
	...
	$iclasses = (_bl_show_internal_classes == true)
        ? $classes['internal']
        : '<div class="bl_nothing"><p>To view PHP internal classes set <em>_bl_show_internal_classes</em> to true</p></div>';
...
////////////////////////////////////
// AJAX DELETE VARS
if (isset($_GET['bl_del']) and _bl_delete_vars === true) {

    // check secret key and IP
	if (!isset($_GET['key']) or $_GET['key'] != _bl_secrect_key) {
        die('error-key');
	}else if (bl_check_ip() == false or _bl_debug_on == false) {
	   die('error');
	}
...
////////////////////////////////////
// FILE BROWSER
if (isset($_GET['bl_file']) and _bl_file_browser == true) {
    ...
    $file = strip_tags(trim($_GET['bl_file']));
    ...
    if (file_exists($file)) {
        ...
        foreach ($file_contents as $line) {
            ...
            $num_color = '#eee';
            if ($num_line == $file_line) {
                $num_color = '#f22';
            }
            $lines_num .= '<span id="line_'.$num_line.'" style="display:inline-block; text-align:right; padding-right:5px; width:55px; border-right:1px solid #ccc; background-color:'.$num_color.';">'.$num_line.'</span><br />';
            $lines_file .= $line;
        }

        $result = '<table>
            <tr>
                <td><code>'.$lines_num.'</code></td>
                <td><code>'.$lines_file.'</code></td>
            </tr>
        </table>';
        ...
        $header = '<div id="bl_header_browser">
            ...
            <p style="float:left;">'.$file.':'.$file_line.'<p>
        </div>';

        die($header . '<div id="bl_file_explorer">'.$result.'</div>');
