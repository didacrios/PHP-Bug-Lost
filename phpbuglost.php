...
## - MESSAGES/LOG PANEL
	 ...
	 * Set to "all" (default) to include all types or comma separated for individual types.
...
## - TIMES PANEL
	...
	/** @type bool Automatically add time marks to any event (log messages, querys, profiles...) */
...
/**
 * bl_link_file()
 * Convert a text file name in a link
 */
	   ...
	   if (is_array($trace) and count($trace)) {
    		...
    		foreach ($trace as $item) {
				...
				if (isset($item['file'])) {
					...
					$function = ($item['function'] != 'bl_error_handler') ? $item['function'] . '()' : '-';
...
function bl_query($query, $con = null) {
	...
	if (_bl_db == 'mysql') {
		...
		if ($con != null) {
			...
			bl_error('PHPBugLost: Require a SQlite object in bl_query() second parameter.
				See docs for use sqlite with PBL', $_SERVER['SCRIPT_FILENAME'], '0');
		}
		...
		if ($con != null) {
			...
			throw new Exception('PHPBugLost: Require a PDO object. See docs for use PDO with PBL');
		}
...
function bl_debug($active = false, $return = false) {
	...
	// You can modify this HTML tags for customize the panel aparence.
