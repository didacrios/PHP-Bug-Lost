...
function bl_keydown(e) {
	...
	if (target === 'html' || target === 'body') {
		...
		} else if (e.keyCode === bl_key_eval) {
			bl_debug_set_panel('eval');
...
/**
 * Change the active eval panel
 * @size string code|result
 */
function bl_debug_set_eval(panel) {
	// obtener todos los links del menu y eliminar la clase activo
	var i,
		e,
		allHTMLTags = document.getElementsByTagName("a");
	...
	for (i = 0; i < allHTMLTags.length; i += 1) {
		if (allHTMLTags[i].className.search('bl_debug_eval_btn') !== -1) {
			allHTMLTags[i].className = 'bl_debug_eval_btn';
		}
	}
	allHTMLTags = document.getElementsByTagName("div");
	for (i = 0; i < allHTMLTags.length; i += 1) {
		if (allHTMLTags[i].className.search('bl_debug_eval_panel') !== -1) {
			allHTMLTags[i].className = 'bl_debug_eval_panel';
		}
	}
	// añadir la clase al elemento actual
	e = document.getElementById('bl_debug_eval_btn_' + panel);
	e.addClass('bl_debug_eval_btn_activo');
	// activar el panel de elemento
	e = document.getElementById('bl_debug_eval_' + panel);
	e.addClass('bl_debug_eval_panel_activo');
}


/**
 * Change the active profile panel
 * @size string code|result
 */
function bl_debug_set_profile(panel) {
	// obtener todos los links del menu y eliminar la clase activo
	var i,
		e,
		allHTMLTags = document.getElementsByTagName("a");
	...
	for (i = 0; i < allHTMLTags.length; i += 1) {
		if (allHTMLTags[i].className.search('bl_debug_profile_btn') !== -1) {
			allHTMLTags[i].className = 'bl_debug_profile_btn';
		}
	}
	allHTMLTags = document.getElementsByTagName("div");
	for (i = 0; i < allHTMLTags.length; i += 1) {
		if (allHTMLTags[i].className.search('bl_debug_profile_panel') !== -1) {
			allHTMLTags[i].className = 'bl_debug_profile_panel';
		}
	}
	// añadir la clase al elemento actual
	e = document.getElementById('bl_debug_profile_btn_' + panel);
	e.addClass('bl_debug_profile_btn_activo');
	// activar el panel de elemento
	e = document.getElementById('bl_debug_profile_' + panel);
	e.addClass('bl_debug_profile_panel_activo');
}
...
////////////////////////////////////
//// LOAD FILES | VIEW SOURCE

function bl_load_file(file, line, url, key, url_var_name) {

	if (!line) {
		line = 0;
	}
	url = url + "?" + url_var_name + "=" + file + "&line=" + line + "&key=" + key;

	$bl('bl_loading').style.display = 'block';
	$bl('bl_file_container').innerHTML = '';
	...
	ajax.onreadystatechange = function () {
		...
		if (ajax.readyState === 4) {
			...
			if (ajax.responseText === 'error-key') {
				alert('There\'re a problem with your secret key');

			} else if (ajax.responseText === 'error-file') {
				...
				$bl('bl_file_container').scrollTop = 0;
				$bl('bl_file_container').innerHTML = ajax.responseText;
				$bl('bl_file_container').style.display = 'block';
				scroll = parseInt(line) - 10;
				$bl('line_' + scroll).scrollIntoView();
...
////////////////////////////////////
//// EVAL CODE | VIEW SOURCE
bl_listen('submit', 'bl_eval_form', bl_eval_code, 'bl_eval_form');
function bl_eval_code() {

	var content = $bl('bl_eval_code').value,
		url = $bl('bl_url').value,
		key = $bl('bl_secret_key').value,
		url_var_name = $bl('bl_eval_url_name').value;

	url = url + "?" + url_var_name + "=" + content + "&bl_key=" + key;
	...
	ajax.onreadystatechange = function () {
		...
		if (ajax.readyState === 4) {
			...
			if (ajax.responseText === 'error-key') {
				...
				$bl('bl_debug_eval_result').innerHTML = ajax.responseText;
				bl_debug_set_eval('result');
...
function bl_msg_ajax_end(data, el_id) {
	var span_error,
		div,
		span = document.getElementById('bl_c_' + el_id);
	span.innerHTML = '<span id="bl_c_c_' + el_id + '">' + data.status + ' ' + data.statusText + '</span> ' + time(data.time) + 's';
	span_error = document.getElementById('bl_c_c_' + el_id);
